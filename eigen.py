...
for (r,c) in zip(mat_row, mat_col):
	if r - c == 1:
		W_quad = [quad_Ynew[np.searchsorted(z_new, z_new[int((r-1)/2)]-z_new[w_q])] for w_q in range(int(((r-1)/2)))] # Generates list with the different terms to be summed for quadrupolar wake
		wake_data[index] = np.sum(W_quad)
		#wake_data[index] = 0. #Uncomment this line for no detuning case
	else:
		wake_data[index] = dip_Ynew[np.searchsorted(z_new, z_new[int((r-1)/2)]-z_new[int(c/2)])] # Generates the dipolar wake term
	index = index + 1
...
l_im = np.array([(0.5*np.imag((eig_val[2*i] - eig_val[1+(2*i)])/np.sqrt((eig_val[2*i] * eig_val[1+(2*i)])))) for i in range(n_bunches)]) # Imaginary part of the eigenvalues
