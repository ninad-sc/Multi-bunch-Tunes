...
dia_ele1 = np.cos(mu - damper_gain) + (alpha * np.sin(mu)) 
...
#full_mat = (I + W)@T
full_mat = T + (W@T) # New overall transfer matrix
...
bunches = [i for i in range(n_bunches)]
plt.figure(5)
plt.scatter(bunches,tunes,color='b',label='Only driving')

plt.figure(7)
plt.plot(x_m) # Plot mean of all bunches
...
I = np.identity(2*n_bunches)
#full_mat = (I + W)@T 
full_mat = T + (W@T) # Addition of the two matrices
turns_mat = np.identity(2*n_bunches)
# Turn by turn tracking
for n in range(n_turns):
	turns_mat = full_mat @ turns_mat
...
bunches = [i for i in range(n_bunches)]
plt.figure(5)
plt.scatter(bunches,tunes,color='g',label='Only detuning')
...
plt.figure(9)
plt.plot(x_pos.T) # Plot all the bunch positions
...
bunches = [i for i in range(n_bunches)]
plt.figure(5)
plt.scatter(bunches,tunes,color='r',label='Driving + detuning')

plt.title('X tunes vs bunches',fontsize=18)
plt.ylabel('Fractional tune',fontsize=18)
#plt.ylim((0.173, 0.181))
plt.xlabel('Bunch number',fontsize=18)
